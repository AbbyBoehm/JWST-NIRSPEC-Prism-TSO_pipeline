    ...
    while not had_KeyError:
        ...
        try:
            for key in special_keys:
                planet[key] = s5_config[key+str(planet_N)] # as long as a planet of this number exists, s5_config will have this key
        except KeyError:
...
def make_flares(s5_config):
    ...
    """
    # Recall our special keys from read_config.
    special_keys = [] # FIX: i haven't defined these yet oops
    prior_keys = [key+"_prior" for key in special_keys]
    for key in prior_keys:
        special_keys.append(key)
    ...
    while not had_KeyError:
        ...
        try:
            for key in special_keys:
                flare[key] = s5_config[key+str(flare_ID)] # as long as a flare of this number exists, s5_config will have this key
        except KeyError:
